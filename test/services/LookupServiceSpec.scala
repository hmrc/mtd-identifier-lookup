  ...
  "calling .getMtdId function" when {
    "a known MTD NINO is passed and not available in lookup cache" should {
      "save a valid mtdId in the lookup cache and return the mtdId" in new Test {
        ...
        private val result: Either[MtdError, MtdIdResponse] = await(target.getMtdId(nino, false, false))
    ...
    "a NotFoundError is returned" should {
      ...
      "transform the error into a Not Enrolled error" in new Test {
        ...
        private val result: Either[MtdError, MtdIdResponse] = await(target.getMtdId(nino, true, true))
